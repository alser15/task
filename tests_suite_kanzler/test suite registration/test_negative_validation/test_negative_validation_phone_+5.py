...
def test_twenty_five():
  ...
  er = web.find_element_by_xpath("//div[@class='index-module__error___JxOWR']")
  ...
  assert 'Обязательное поле. Телефон должен иметь формат: +999999999. Допускается до 11 цифр.' == find_text, 'Поле не найдено!' # 'Поле не найдено!' срабатывает, если в find_text пусто
  web.quit()
