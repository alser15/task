...
import string
def test_thirteen():
  #Генерация слова из 301 символа
  word = ''
  for i in range(1,32):
    word+='в'
    i+=1
  ...
  # Заполнение формы "Город"
  string4 = shadow_root4.find_element_by_name('city')
  string4.send_keys('Воронеж')
  # Заполнение формы "Почта"
  string5 = shadow_root5.find_element_by_name('email')
  string5.send_keys(random_char(245)+"@gmail.com")
  time.sleep(2)
  ...
  webdriver.ActionChains(web).move_to_element(string12 ).click(string12 ).perform()
  string12.click()
  time.sleep(15)
  # Ищем надпись
  x = web.find_element_by_xpath("//gx-input[@name='email']/div[@slot='help-text']")
  # Скролим окно до момента появления надписи об ошибке"
  web.execute_script("return arguments[0].scrollIntoView(true);", x)
  ...
  assert 'Убедитесь, что это значение содержит не более 254 символов (сейчас 255).' == find_text, 'Поле не найдено!' # 'Поле не найдено!' срабатывает, если в find_text пусто
  web.quit()
